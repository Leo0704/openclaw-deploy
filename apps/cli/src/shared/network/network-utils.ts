...
const { createError, fromNativeError, ErrorType, Errors } = require('../errors/error-utils');
...
export interface NetworkProbeSummary {
  githubDirectConnected: boolean;
  bestSource: SourceProbeResult | null;
  sortedSources: SourceProbeResult[];
  probeTime: number;
}
...
async function probeSource(
  ...
  return {
    name,
    url: testUrl,
    type: name === 'GitHub' ? 'direct' : 'mirror',
    connected: result.success,
    latency: result.success ? latency : undefined,
    error: result.error?.message,
  };
...
/**
 * 并行探测所有镜像源，返回排序结果
 * @param probeTimeout 单个源的超时时间（毫秒）
 */
export async function probeNetworkSources(
  probeTimeout: number = 5000
): Promise<NetworkProbeSummary> {
  const probeStartTime = Date.now();

  // 定义要探测的源
  const sources = [
    { name: 'GitHub', testUrl: 'https://github.com' },
    { name: 'GitMirror', testUrl: 'https://hub.gitmirror.com' },
    { name: 'GHProxy', testUrl: 'https://mirror.ghproxy.com' },
  ];

  // 并行探测所有源
  const results = await Promise.all(
    sources.map(s => probeSource(s.name, s.testUrl, probeTimeout))
  );
  ...
  const githubDirect = results.find(r => r.name === 'GitHub');
  const bestSource = sorted.find(r => r.connected) || null;

  return {
    githubDirectConnected: githubDirect?.connected || false,
    bestSource,
    sortedSources: sorted,
    probeTime: Date.now() - probeStartTime,
  };
}

/**
 * 根据网络探测结果，返回推荐的依赖安装策略
 */
export function getRecommendedInstallStrategy(probeResult: NetworkProbeSummary): {
...
export async function downloadFile(
  ...
  return new Promise((resolve) => {
    ...
    // 辅助函数：安全 resolve
    const safeResolve = (result: { success: boolean; error?: Error; bytesWritten?: number }) => {
      if (settled) return;
      settled = true;
      resolve(result);
    };
    ...
    const req = client.get(url, {
      ...
      if (res.statusCode && res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
        ...
        // 添加 .catch() 处理未捕获的 Promise rejection
        downloadFile(redirectedUrl, destPath, { ...options, _redirectCount: _redirectCount + 1 })
          .then(safeResolve)
          .catch((err: Error) => {
            safeResolve({
              success: false,
              error: fromNativeError(err, ErrorType.NETWORK, { url: redirectedUrl, destPath }),
            });
          });
        return;
      }
      ...
      file.on('finish', () => {
        ...
        safeResolve({ success: true, bytesWritten });
      });
      ...
      file.on('error', (err: Error) => {
        ...
        safeResolve({
          success: false,
          error: fromNativeError(err, ErrorType.NETWORK, { url, destPath }),
        });
      });
    });
