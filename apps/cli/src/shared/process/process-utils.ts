...
const {
  ...
  ErrorType,
  createError,
} = require('../errors/error-utils') as typeof import('../errors/error-utils');
...
export function runCommand(
  ...
  while (attempt <= retries) {
    ...
    try {
      ...
      const error = e as { stderr?: unknown; stdout?: unknown; message?: string; status?: unknown };
      ...
      // Safely extract exitCode - only use status if it's a number
...
export function runCommandArgs(
  ...
  try {
    ...
    const appError = createError(ErrorType.PROCESS, 'PROCESS_ERROR', {
      userMessage: `命令执行失败: ${errorMessage}`,
      context: { file, args, cwd, exitCode },
    });
...
// Re-export from platform-specific implementation to avoid code duplication
export { resolveSpawnExecutable } from '../../platform/process/resolve-executable';
